...
int main(int argc, char* argv[])
{
  ...
  uint64_t weight_phys = RK_MPI_MMZ_Handle2PhysAddr(weight_mb);
  if (weight_phys == 0) {
    printf("RK_MPI_MMZ_Handle2PhysAddr failed!\n");
    return -1;
  }
  weight_mem = rknn_create_mem_from_phys(ctx, weight_phys, weight_virt, mem_size.total_weight_size);
  ret        = rknn_set_weight_mem(ctx, weight_mem);
  if (ret < 0) {
    printf("rknn_set_weight_mem fail! ret=%d\n", ret);
    return -1;
  }
  printf("weight mb info: virt = %p, phys = %#lx, size: %d\n", weight_virt, weight_phys, mem_size.total_weight_size);
  ...
  uint64_t internal_phys = RK_MPI_MMZ_Handle2PhysAddr(internal_mb);
  if (internal_phys == 0) {
    printf("RK_MPI_MMZ_Handle2PhysAddr failed!\n");
    return -1;
  }
  internal_mem = rknn_create_mem_from_phys(ctx, internal_phys, internal_virt, mem_size.total_internal_size);
  ret          = rknn_set_internal_mem(ctx, internal_mem);
  if (ret < 0) {
    printf("rknn_set_internal_mem fail! ret=%d\n", ret);
    return -1;
  }
  printf("internal mb info: virt = %p, phys = %#lx, size: %d\n", internal_virt, internal_phys,
